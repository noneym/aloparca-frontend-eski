...
const ProductCard = ({ product, isCart, dispatch, onClose, swiperLazy }) => {
  ...
  const [imgSRC, setSRC] = useState(handleParseImage(product.gorsel)?.[0]);
  const _imgSRC = `https://resize.aloparca.com/upload/w_260,h_200,pns/v2/${imgSRC}`;
  ...
  return (
    <Outer isCart={isCart} id="for-hover">
      <Link
        to={`/yedek-parca-v2${product.slug}`}
        className="link"
        title={product.name}
      />
      {swiperLazy ? (
