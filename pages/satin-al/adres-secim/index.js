...
class CheckoutAddress extends React.Component {
  ...
  async componentDidMount() {
    ...
    //   if (teslimat.length) {
    //     dispatch(
    //       setAddress({ id: teslimat[teslimat.length - 1].Id, type: 'teslimat' }),
    //     );
    //     dispatch(
    //       setAddress({ id: fatura[fatura.length - 1].Id, type: 'fatura' }),
    //     );
    //   }
    // }
  }
          ...
          !address.teslimat || !address.fatura
            ? () => {
              this.props.dispatch({
                type: 'FLASH_MESSAGE',
                payload:
                    'Lütfen teslimat ve fatura adreslerini giriniz seçiniz.',
              });
            }
            : null
        }
      >
        <Outer width={1} p={[2, 2, 3]} flexDirection="column">
          ...
          <Flex mx={[1, 1, 3]} my={1} alignItems="center">
            ...
            <Box mx={[1, 1, 2]}>
              <img
                src="/static/img/yurtici-kargo.svg"
                width="100"
                alt="Yurtiçi Kargo"
              />
            </Box>
            <Box className="kargo-text">
              Yurtiçi Kargo (
              {cart && cart.kargo}
                TL)
            </Box>
...
const mapStateToProps = ({ userData, address, cart }) => ({
  userData,
  address,
  cart,
});
