...
import Api from '../api';
import { checkCart, deleteAllCart } from '../actions';
...
class KrediKarti extends React.Component {
  ...
  kumbaradanDus = (e) => {
    ...
    let numbE = data < 0 ? 0 : data;

    let cartSubTotal = Number(this.props.cart.subtotal);
  ...
  calculateKumb = (tutar_) => {
    let kumbara = this.state.kumbaraTutar.toFixed(2);
    let tutar = Number(tutar_).toFixed(2);
    let fark = kumbara - tutar;
  ...
  modalOpenB2B = async () => {
    ...
    if (!formError) {
      ...
      // Sadece Kumbara ise ise ödeme tipi : 15 ekleyecek sipariş detay formuna
      ...
      if (odemetipi_ === '5') {
        ...
        let status = true;
        let html = "Ödeme kumbaradan yapıldı";
  ...
  render() {
    ...
    const { cart, dispatch, kumbaraOk, kumbLess, kumbKes, kkartKes } = this.props;
...
class Havale extends React.Component {
  ...
  render() {
    ...
    const { bankChecked, b2bAccess, isKumbaraChecked, kumbaraTutar, kumbaradanDus } = this.state;
              ...
              banka.map(item => (
                ...
                >
                  ...
                  >
                    ...
                    <Flex flexDirection="column">
                      <Box mb={1}>
                        <strong>Banka:</strong> {item.Banka}
                      </Box>
                      <Box mb={1}>
                        <strong>IBAN:</strong> {item.iban}
                      </Box>
                      <Box mb={1}>
                        <strong>Şube Adı:</strong> {item.Sube}
                      </Box>
                      <Box mb={1}>
                        <strong>Şube Kodu:</strong> {item.Sube_Kodu}
                      </Box>
                      <Box mb={1}>
                        <strong>Hesap No:</strong> {item.Hesap_No}
                      </Box>
                      <Box>
                        <strong>Alıcı:</strong> {item.Alici}
                      </Box>
                    </Flex>
                  </Flex>
                </Flex>
...
class Taksitler extends React.Component {
  ...
  render() {
    const { cart, taksit, taksitFunc, kkartKes } = this.props;
  ...
  panes = [
    {
      menuItem: (
        ...
        <span />{site === "b2b" ? " Kredi / Banka Kartı - Kumbara" : " Kredi / Banka Kartı" }
        </Menu.Item>
      ),
      render: () => (
    ...
    },
    {
      menuItem: (
        ...
        <span />{site === "b2b" ? " Havale / EFT - Kumbara" : " Havale / EFT" }
        </Menu.Item>
      ),
      render: () => (
        <Tab.Pane attached={false}>
        ...
        {site === "b2b" ? 
          ...
          kumbKes={this.props.kumbKes} //deneme
          kkartKes={this.props.kkartKes} //deneme
          havaledeKumb={this.props.havaledeKumb} //deneme
          /> : 
